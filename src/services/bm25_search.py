    ...
    async def build_index(self, chunks: List[CodeChunk]) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for doc_id, length in document_lengths.items():
                ...
                conn.execute(
                    "INSERT INTO document_stats (document_id, document_length, file_path, language) VALUES (?, ?, ?, ?)",
    ...
    async def search(self, query: str, language: str = None, max_results: int = 50) -> List[Tuple[str, float]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            for term in query_terms:
                # Get term statistics
                cursor = conn.execute(
                    "SELECT document_frequency, posting_list FROM bm25_index WHERE term = ?",
                    (term,)
                )
                result = cursor.fetchone()
                ...
                for doc_id, term_freq in posting_list:
                    # Get document length
                    cursor = conn.execute(
                        "SELECT document_length FROM document_stats WHERE document_id = ?",
                        (doc_id,)
                    )
                    ...
                    if language:
                        cursor = conn.execute(
                            "SELECT language FROM document_stats WHERE document_id = ?",
                            (doc_id,)
                        )
    ...
    async def _add_chunks_to_index(self, chunks: List[CodeChunk]) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            for chunk in chunks:
                ...
                # Store document stats
                conn.execute(
                    "INSERT OR REPLACE INTO document_stats (document_id, document_length, file_path, language) VALUES (?, ?, ?, ?)",
                ...
                for term, freq in term_counts.items():
                    ...
                    if result:
                        ...
                        updated = False
                        for i, (did, f) in enumerate(posting_list):
                            if did == doc_id:
...
class CodeTokenizer:
    ...
    def __init__(self):
        ...
        self.stop_words = {
            ...
            'if', 'else', 'elif', 'while', 'for', 'def', 'class', 'import', 'from', 'as',
            'try', 'except', 'finally', 'with', 'pass', 'break', 'continue', 'return',
            'function', 'var', 'let', 'const', 'new', 'this', 'super', 'extends', 'implements',
            'public', 'private', 'protected', 'static', 'abstract', 'final', 'override'
        }
        
        # Language-specific keywords to exclude
        self.language_keywords = {
            'python': {'def', 'class', 'if', 'else', 'elif', 'while', 'for', 'try', 'except', 'finally', 'with', 'as', 'import', 'from'},
            'javascript': {'function', 'var', 'let', 'const', 'if', 'else', 'while', 'for', 'try', 'catch', 'finally', 'class', 'extends'},
            'typescript': {'function', 'var', 'let', 'const', 'if', 'else', 'while', 'for', 'try', 'catch', 'finally', 'class', 'extends', 'interface', 'type'},
            'java': {'public', 'private', 'protected', 'static', 'final', 'abstract', 'class', 'interface', 'extends', 'implements', 'if', 'else', 'while', 'for', 'try', 'catch'},
            'go': {'func', 'var', 'const', 'type', 'struct', 'interface', 'if', 'else', 'for', 'switch', 'case', 'default', 'go', 'defer'},
    ...
    def tokenize(self, content: str, language: str = None) -> List[str]:
        ...
        for token in processed_tokens:
            if (len(token) > 2 and 
                ...
                token not in language_stops and
                token.isalnum() and
                not token.isdigit()):
