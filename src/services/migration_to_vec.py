...
class EmbeddingsMigrator:
    ...
    def create_vec_database(self) -> bool:
        ...
        try:
            ...
            success = self.vec_loader.create_vec_table(
                conn, "embeddings_vec", dimension=self.config.VEC_DIMENSION, index_type=self.config.VEC_INDEX_TYPE
            )
    ...
    def migrate_batch(
        self, conn_legacy: sqlite3.Connection, conn_vec: sqlite3.Connection, offset: int, batch_size: int
    ) -> int:
        """
        ...
        for row in rows:
            try:
                ...
                cursor_vec = conn_vec.execute("INSERT INTO embeddings_vec(embedding) VALUES (?)", (embedding_bytes,))
    ...
    def migrate(self, batch_size: int = 100) -> bool:
        ...
        while offset < with_vectors:
            if self.dry_run:
                ...
                logger.info("[DRY RUN] Would migrate batch %d-%d", offset + 1, offset + batch_count)
                total_migrated += batch_count
            else:
                batch_migrated = self.migrate_batch(conn_legacy, conn_vec, offset, batch_size)
                total_migrated += batch_migrated
                logger.info(
                    f"Migrated batch {offset // batch_size + 1}: "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate existing embeddings to sqlite-vec format")
    parser.add_argument("--dry-run", action="store_true", help="Simulate migration without making changes")
    parser.add_argument(
        "--batch-size", type=int, default=100, help="Number of embeddings to migrate per batch (default: 100)"
    )
