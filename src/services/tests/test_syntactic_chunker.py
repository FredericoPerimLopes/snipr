...
from ..syntactic_chunker import SyntacticChunker, ChunkingConfig
...
class TestSyntacticChunker:
    ...
    async def test_group_children_by_size(self, chunker):
        ...
        with patch.object(chunker, '_get_node_char_count') as mock_char_count:
            ...
            large_node_group = next((group for group in groups if len(group) == 1 and group[0].type == "large_function"), None)
