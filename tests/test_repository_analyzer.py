...
class TestModuleGraph:
    def test_graph_creation(self):
        ...
        node = RepoNode(id="test_node", type="function", name="test_func", file_path="/test.py", metadata={})
...
class TestContextualReranker:
    ...
    async def test_rerank_with_repository_context(self, sample_chunks):
        ...
        with patch.object(reranker, "_calculate_context_score", return_value=0.8):
            reranked = await reranker.rerank_with_repository_context(sample_chunks, "test query")
...
class TestRepositoryAnalyzerIntegration:
    ...
    async def test_add_inheritance_relationships(self, sample_chunks):
        ...
        # Add class nodes
        user_manager_node = RepoNode("class:/test/auth.py:UserManager", "class", "UserManager", "/test/auth.py", {})
