...
class TestSearchQualityMetrics:
    ...
    async def test_evaluate_query_results(self, sample_chunks, benchmark_query):
        ...
        result = await metrics._evaluate_query_results(benchmark_query, sample_chunks, query_time=50.0)
    ...
    async def test_evaluate_search_method(self, sample_chunks):
        ...
        result = await metrics.evaluate_search_method(mock_search, "test_method", test_queries)
...
class TestCodeRetrievalMetrics:
    def test_semantic_relevance_score(self, sample_chunks):
        ...
        score = CodeRetrievalMetrics.semantic_relevance_score("user authentication function", chunk)
        assert score > 0.5

        # Low relevance query
        score = CodeRetrievalMetrics.semantic_relevance_score("database operations", chunk)
        assert score < 0.5
...
class TestPerformanceBenchmark:
    ...
    async def test_benchmark_search_methods(self, sample_chunks):
        ...
        search_services = {"fast_method": fast_search, "slow_method": slow_search}

        results = await benchmark.benchmark_search_methods(search_services, ["test query"])
    ...
    async def test_compare_search_methods(self, sample_chunks):
        ...
        with patch("src.services.quality_metrics.SearchQualityMetrics.evaluate_search_method") as mock_eval:
            mock_eval.side_effect = [
                {"method": "baseline", "aggregate_metrics": {"avg_precision_at_5": 0.6, "avg_recall_at_5": 0.5}},
                {"method": "enhanced", "aggregate_metrics": {"avg_precision_at_5": 0.8, "avg_recall_at_5": 0.7}},
            ]

            comparison = await dashboard.compare_search_methods(baseline_service, enhanced_service, ["test query"])
...
async def test_integration_quality_metrics(sample_chunks):
    ...
    # Test evaluation
    result = await metrics.evaluate_search_method(mock_search_function, "mock_search")
...
async def test_quality_metrics_with_empty_results():
    ...
    result = await metrics.evaluate_search_method(empty_search_function, "empty_search")
...
async def test_quality_metrics_error_handling():
    ...
    result = await metrics.evaluate_search_method(failing_search_function, "failing_search")
